...
import me.MiniDigger.RideThaMob.RideThaMobPlugin;
 ...
 * href="http://minecraft.gamepedia.com/Commands#tellraw">tellraw command</a>. <p> This class
 * follows the builder pattern, allowing for method chaining. It is set up such that invocations of
 * property-setting methods will affect the current editing component, and a call to {@link #then()}
 * or {@link #then(String)} will append a new editing component to the end of the message,
 * optionally initializing it with text. Further property-setting method calls will affect that
 * editing component. </p>
 */
public class FancyMessage implements JsonRepresentedObject, Cloneable, Iterable<MessagePart>, ConfigurationSerializable {
     ...
     * when the client hovers over the text. <p> Tooltips do not inherit display characteristics,
     * such as color and styles, from the message component on which they are applied. </p>
     *
     ...
     * the client hovers over the text. <p> Tooltips do not inherit display characteristics, such as
     * color and styles, from the message component on which they are applied. </p>
     *
     * @param itemJSON A string representing the JSON-serialized NBT data tag of an {@link
     *                 ItemStack}.
     * @return This builder instance.
     */
     ...
     * over the text. <p> Tooltips do not inherit display characteristics, such as color and styles,
     * from the message component on which they are applied. </p>
     *
     ...
     * hovers over the text. <p> Tooltips do not inherit display characteristics, such as color and
     ...
     * text when the client hovers over the text. <p> Tooltips do not inherit display
     * characteristics, such as color and styles, from the message component on which they are
     * applied. </p>
     *
    ...
    public FancyMessage formattedTooltip(final List<FancyMessage> lines) {
        return formattedTooltip(lines.toArray(new FancyMessage[lines.size()]));
    }
     ...
     * to send this message to clients without JSON formatting support. <p> Serialization of this
     * message by using this message will include (in this order for each message part): <ol>
     * <li>The color of each message part.</li> <li>The applicable stylizations for each message
     * part.</li> <li>The core text of the message part.</li> </ol> The primary omissions are
     * tooltips and clickable actions. Consequently, this method should be used only as a last
     * resort. </p> <p> Color and formatting can be removed from the returned string by using {@link
     * ChatColor#stripColor(String)}. </p>
     *
     ...
     */
    public static FancyMessage deserialize(final String json) throws MalformedJsonException {
        ...
        for (final JsonElement mPrt : extra) {
            ...
            for (final Map.Entry<String, JsonElement> entry : messagePart.entrySet()) {
                ...
                if (TextualComponent.isTextKey(entry.getKey())) {
                    ...
                    final Map<String, Object> serializedMapForm = new HashMap<String, Object>(); // Must be object due
                    // to
                    // Bukkit serializer API
                    // compliance
    ...
    private void send(final CommandSender sender, final String jsonString) {
        ...
        RideThaMobPlugin.getInstance().getFancyMessageSender().send((Player) sender, jsonString);
    }
}
